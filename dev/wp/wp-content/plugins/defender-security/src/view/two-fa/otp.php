...
<head>
	...
	<title>
		...
		if ( is_multisite() ) {
			...
			$login_header_title = esc_html__( 'Powered by WordPress', 'defender-security' );
		}
...
END;
	...
	if ( is_array( $providers ) && array() !== $providers ) {
		...
		if ( ( is_array( $providers ) || $providers instanceof Countable ? count( $providers ) : 0 ) > 1 ) {
			?>
				<div id="wrap-nav">
					<p><?php esc_attr_e( 'Having problems? Try another way to log in', 'defender-security' ); ?></p>
					<ul id="nav">
	...
	if ( ! function_exists( 'login_footer' ) ) {
		...
		">
					...
					printf(
						...
						esc_html_x( '&larr; Back to %s', 'site', 'defender-security' ),
