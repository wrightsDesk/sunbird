...
class Webauthn extends Two_Factor_Provider {
	...
	public function get_user_label(): string {
		return esc_html__( 'Web Authentication', 'defender-security' );
	}
	...
	public function authentication_form() {
		...
		<div class="welcome-screen">
			...
			<p><?php esc_html_e( 'Select an authentication method to continue.', 'defender-security' ); ?></p>
			<div class="option-row" data-authType="platform">
				<span class="icon biometric"></span>
				<span>
					<strong class="option-hd"><?php esc_html_e( 'Biometric Authentication', 'defender-security' ); ?></strong>
					<?php esc_html_e( 'Use fingerprint or facial recognition', 'defender-security' ); ?>
				</span>
			</div>
			<div class="option-row" data-authType="cross-platform">
				<span class="icon hardware"></span>
				<span>
					<strong class="option-hd"><?php esc_html_e( 'Hardware Key Authentication', 'defender-security' ); ?></strong>
					<?php esc_html_e( 'Use USB security keys to login', 'defender-security' ); ?>
				</span>
			</div>
		</div>
		<div class="webauthn-content-wrap">
			<div class="webauthn-platform" style="display:none;">
				<p class="wpdef-2fa-label"><?php esc_html_e( 'Biometric Authentication', 'defender-security' ); ?></p>
			...
			<div class="webauthn-cross-platform" style="display:none;">
				<p class="wpdef-2fa-label"><?php esc_html_e( 'Hardware Key Authentication', 'defender-security' ); ?></p>
				<p class="wpdef-2fa-text">
					<?php esc_html_e( 'Insert and tap on your USB security key to login.', 'defender-security' ); ?>
				</p>
			</div>
			<p class="wpdef-2fa-text wpdef-2fa-biometric-process-desc" style="display: none;">
				<img class="def-loader" src="<?php defender_asset_url( '/assets/img/spinner.svg', true ); ?>"
					alt="loading">
				<?php esc_html_e( 'Verification in process...', 'defender-security' ); ?>
			</p>
			<div class="wpdef-2fa-webauthn-control" style="display:none;">
				<button type="button" class="button"
						id="wpdef-2fa-webauthn-back-btn"><?php esc_html_e( 'Back', 'defender-security' ); ?></button>
				<button type="button" class="button button-primary float-r" id="wpdef-2fa-biometric-retry-btn">
					<span class="dashicons dashicons-update"></span>
					<?php esc_html_e( 'Retry', 'defender-security' ); ?>
				</button>
			</div>
	...
	private function missed_requirement_message(): array {
		...
		if ( ! $this->is_ssl() ) {
			...
			$warning_core_message         .= esc_html__( 'SSL certificate or HTTPS is not forced. ', 'defender-security' );
		}
	...
	private function concatenate_failed_extensions( array $extensions ): string {
		...
		if ( count( $extensions ) > 1 ) {
			$last_extension = esc_html__( ' and ', 'defender-security' ) . array_pop( $extensions );
