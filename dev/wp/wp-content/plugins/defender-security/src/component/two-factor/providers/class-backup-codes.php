...
class Backup_Codes extends Two_Factor_Provider {
	...
	public function get_label(): string {
		return esc_html__( 'Backup Codes', 'defender-security' );
	}
	...
	public function authentication_form(): void {
		...
		<p class="wpdef-2fa-text">
			<?php echo esc_html__( 'Enter one of your recovery codes to log in to your account.', 'defender-security' ); ?>
		</p>
		<input type="text" autofocus value="" autocomplete="off" name="backup-codes">
		<button class="button button-primary float-r"
				type="submit"><?php esc_attr_e( 'Authenticate', 'defender-security' ); ?></button>
		<?php
	}
				...
				<span>
					<?php
						...
						/* translators: %s: URL to regenerate code */
							...
							'<a href="' . esc_url( get_edit_user_link( $user->ID ) . '#wpdef-2fa-backup-codes' ) . '">' . esc_html__( 'generate', 'defender-security' ) . '</a>'
						);
					?>
				<span>
			</p>
	...
	public static function display_number_of_codes( int $count ): string {
		return sprintf(
		/* translators: %s: count */
			_n( '%s unused code remaining', '%s unused codes remaining', $count, 'defender-security' ),
			$count
		);
	}
	...
	public function validate_authentication( WP_User $user ) {
		...
		if ( false === $codes ) {
			return new WP_Error(
				'opt_fail',
				esc_html__( 'ERROR: Invalid backup code.', 'defender-security' )
			);
		}
