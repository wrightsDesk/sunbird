...
class Totp extends Two_Factor_Provider {
	...
	public function get_label(): string {
		return esc_html__( 'TOTP Authenticator App', 'defender-security' ) . $this->label;
	}
	...
	public function get_login_label(): string {
		return esc_html__( 'TOTP Authentication', 'defender-security' );
	}
	...
	public function get_user_label(): string {
		return esc_html__( 'TOTP', 'defender-security' );
	}
	...
	public function init_provider( WP_User $user ) {
		...
		$this->label       = $is_on
			? sprintf(
				...
				esc_html__( 'Reset Keys', 'defender-security' )
			)
			: '';
		$this->description = $is_on
			? esc_html__( 'TOTP Authentication method is active for this site', 'defender-security' )
			: esc_html__( 'Use an authenticator app to sign in with a separate passcode.', 'defender-security' );
	}
