...
class Mail extends Component {
	...
	public function get_sender_name( $notification_slug ): string {
		...
		if ( $whitelabel->can_whitelabel() ) {
			...
			$plugin_label = esc_html__( 'Defender', 'defender-security' );
		}
	...
	protected function find_feature_name_by_slug( $slug ) {
		switch ( $slug ) {
			case Tweak_Reminder::SLUG:
				return esc_html__( 'Recommendations', 'defender-security' );
			...
			case 'subscription':
				return esc_html__( 'Subscription', 'defender-security' );
			case 'subscribe_confimed':
				return esc_html__( 'Subscription Confirmed', 'defender-security' );
			case 'unsubscription':
				return esc_html__( 'Unsubscription', 'defender-security' );
			case 'totp':
