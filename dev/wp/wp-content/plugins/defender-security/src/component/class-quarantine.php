...
class Quarantine extends Component {
	...
	private function do_quarantine( Scan_Item $scan_item, string $parent_action ): array {
		...
		if ( ! $is_renamed ) {
			$result = array(
				'message' => sprintf(
					...
					esc_html__( 'Failed to quarantine the file %1$s.', 'defender-security' ),
		...
		if ( ! is_int( $saved ) ) {
			$result = array(
				'message' => sprintf(
					/* translators: 1: Filename with extension */
					esc_html__( 'Failed to add quarantine record in DB for the file %1$s.', 'defender-security' ),
					'<strong>' . $file_name_with_extension . '</strong>'
				),
				'success' => false,
			);
		...
		$message = sprintf(
			/* translators: 1: Filename with extension */
			esc_html__( 'Quarantined the file %1$s and deleted the source file successfully.', 'defender-security' ),
			'<strong>' . $file_name_with_extension . '</strong>'
		);
	...
	public function restore_file( $entity ): array {
		...
		if (
			...
			$result = array(
				'message' => esc_html__( 'Record not exists in DB.', 'defender-security' ),
				'success' => false,
			);
		...
		if ( ! $this->is_quarantine_file_exists( $file_metadata->file_hash ) ) {
			$result = array(
				'message' => esc_html__( 'Quarantined file doesn\'t exist in the quarantine directory.', 'defender-security' ),
				'success' => false,
			);
		...
		if ( array() === $plugin_headers ) {
			$result = array(
				'message' => sprintf(
					/* translators: 1: Filename with extension */
					esc_html__( 'Plugin main file missing for %1$s file.', 'defender-security' ),
					'<strong>' . $file_name_with_extension . '</strong>'
				),
				'success' => false,
			);
		...
		if ( ! $is_renamed ) {
			$result = array(
				'message' => esc_html__( 'Failed to restore quarantined file.', 'defender-security' ),
				'success' => false,
			);
		...
		if ( ! $is_delete_success ) {
			$result = array(
				'message' => esc_html__( 'Failed to remove quarantine record from DB.', 'defender-security' ),
				'success' => false,
			);
		...
		$result = array(
			'message' => sprintf(
				/* translators: 1: Filename with extension */
				esc_html__( 'Restored %1$s', 'defender-security' ),
				'<strong>' . $file_name_with_extension . '</strong>'
			),
			'success' => true,
		);
	...
	private function plugin_write_handler( Quarantine_Model $quarantine_model ): array {
		...
		if ( ! $this->is_likely_wporg_slug( $plugin_directory_name ) ) {
			return array(
				'message' => esc_html__( 'Make sure plugin exists in WordPress plugin repository.', 'defender-security' ),
				'success' => false,
			);
		}
		...
		$generic_error            = array(
			'message' => sprintf(
				/* translators: 1: Filename with extension */
				esc_html__( 'Failed to write the file %1$s.', 'defender-security' ),
				'<strong>' . $file_name_with_extension . '</strong>'
			),
			'success' => false,
		);
		...
		if (
			...
			if ( $is_written ) {
				return array(
					'message' => sprintf(
						/* translators: 1: Filename with extension */
						esc_html__( 'Writes the file %1$s successfully.', 'defender-security' ),
						'<strong>' . $file_name_with_extension . '</strong>'
					),
					'success' => true,
				);
			...
			return array(
				'message' => sprintf(
					...
					esc_html__( 'WordPress remote error: %1$s', 'defender-security' ),
					'<strong>' . $error . '</strong>'
				),
				'success' => false,
			);
	...
	public function quarantine_collection(): array {
		...
		foreach ( $collections as $index => $collection ) {
			...
			$file_modified_timestamp = strtotime( $collection['file_modified_time'] );
			$created_timestamp       = strtotime( $collection['created_time'] );

			$collections[ $index ]['file_modified_time'] = false !== $file_modified_timestamp
				? wp_date( 'M j, Y', $file_modified_timestamp )
				: $collection['file_modified_time'];

			$collections[ $index ]['created_time'] = false !== $created_timestamp
				? wp_date( 'M j, Y', $created_timestamp )
