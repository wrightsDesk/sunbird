...
class Post_Audit extends Audit_Event {
	...
	public function get_hooks(): array {
		return array(
				...
				'text'         => array(
					array(
						...
						/* translators: 1: Blog name, 2: Source of action. For e.g. Hub or a logged-in user, 3: Post type, 4: Post title */
							esc_html__( '%1$s %2$s published %3$s "%4$s"', 'defender-security' ),
							'{{blog_name}}',
							'{{wp_user}}',
							'{{post_type_label}}',
							'{{post_title}}'
					...
					array(
						sprintf(
						/* translators: 1: Blog name, 2: Source of action. For e.g. Hub or a logged-in user, 3: Post type, 4: Post title */
							esc_html__( '%1$s %2$s pending %3$s "%4$s"', 'defender-security' ),
							'{{blog_name}}',
							'{{wp_user}}',
							'{{post_type_label}}',
							'{{post_title}}'
					...
					array(
						sprintf(
						/* translators: 1: Blog name, 2: Source of action. For e.g. Hub or a logged-in user, 3: Post type, 4: Post title */
							esc_html__( '%1$s %2$s drafted %3$s "%4$s"', 'defender-security' ),
							'{{blog_name}}',
							'{{wp_user}}',
							'{{post_type_label}}',
							'{{post_title}}'
					...
					array(
						...
						/* translators: 1: Blog name, 2: Source of action. For e.g. Hub or a logged-in user, 3: Post type, 4: Post title, 5: Old status, 6: New status */
							esc_html__( '%1$s %2$s changed %3$s "%4$s" status from %5$s to %6$s', 'defender-security' ),
							'{{blog_name}}',
							'{{wp_user}}',
							'{{post_type_label}}',
							'{{post_title}}',
			...
			'delete_post'            => array(
				...
				'text'         => sprintf(
				/* translators: 1: Blog name, 2: Source of action. For e.g. Hub or a logged-in user, 3: Post type, 4: Post title */
					esc_html__( '%1$s %2$s deleted %3$s "%4$s"', 'defender-security' ),
					'{{blog_name}}',
					'{{wp_user}}',
					'{{post_type_label}}',
					'{{post_title}}'
			...
			'untrashed_post'         => array(
				...
				'text'         => sprintf(
				/* translators: 1: Blog name, 2: Source of action. For e.g. Hub or a logged-in user, 3: Post type, 4: Post title */
					esc_html__( '%1$s %2$s untrashed %3$s "%4$s"', 'defender-security' ),
					'{{blog_name}}',
					'{{wp_user}}',
					'{{post_type_label}}',
					'{{post_title}}'
			...
			'trashed_post'           => array(
				...
				'text'         => sprintf(
				/* translators: 1: Blog name, 2: Source of action. For e.g. Hub or a logged-in user, 3: Post type, 4: Post title */
					esc_html__( '%1$s %2$s trashed %3$s "%4$s"', 'defender-security' ),
					'{{blog_name}}',
					'{{wp_user}}',
					'{{post_type_label}}',
					'{{post_title}}'
	...
	public function post_updated_callback() {
		...
		if ( true === $is_updated ) {
				...
				if ( wp_json_encode( $post_before ) !== wp_json_encode( $post_after ) ) {
					...
					if ( $post_before['post_title'] !== $post_after['post_title'] && 1 === $item_changed_count ) {
						...
						/* translators: 1: Blog name, 2: User's display name, 3: Post type, 4: Post ID, 5: Old post title, 6: New post title */
							esc_html__( '%1$s %2$s updated %3$s ID %4$d, title from "%5$s" to "%6$s"', 'defender-security' ),
							$blog_name,
						...
						/* translators: 1: Blog name, 2: User's display name, 3: Post type, 4: Post ID, 5: Old post slug, 6: New post slug */
							esc_html__( '%1$s %2$s updated %3$s ID %4$d, slug from "%5$s" to "%6$s"', 'defender-security' ),
							$blog_name,
							$this->get_user_display( get_current_user_id() ),
							$post_type->labels->singular_name,
							$post_after['ID'],
						...
						/* translators: 1: Blog name, 2: User's display name, 3: Post type, 4: Post ID, 5: Old author name, 6: New author name */
							esc_html__( '%1$s %2$s updated %3$s ID %4$d, author from "%5$s" to "%6$s"', 'defender-security' ),
							$blog_name,
							$this->get_user_display( get_current_user_id() ),
							$post_type->labels->singular_name,
							$post_after['ID'],
						...
						/* translators: 1: Blog name, 2: User's display name, 3: Post type, 4: Post title */
							esc_html__( '%1$s %2$s updated %3$s "%4$s"', 'defender-security' ),
							$blog_name,
							$this->get_user_display( get_current_user_id() ),
							$post_type->labels->singular_name,
							$post_after['post_title']
			...
			$text = sprintf(
			/* translators: 1: Blog name, 2: User's display name, 3: Post type, 4: Post title */
				esc_html__( '%1$s %2$s added new %3$s "%4$s"', 'defender-security' ),
				$blog_name,
				$this->get_user_display( get_current_user_id() ),
				$post_type->labels->singular_name,
				$post->post_title
