...
class Mask_Login extends Setting {
	...
	public function labels() {
		return array(
			...
			'mask_url'                 => esc_html__( 'Masking URL', 'defender-security' ),
			'redirect_traffic'         => esc_html__( 'Redirect traffic', 'defender-security' ),
			'redirect_traffic_url'     => esc_html__( 'Redirection URL', 'defender-security' ),
	...
	protected function after_validate(): void {
		if ( '' !== $this->mask_url ) {
			...
			if ( in_array( $this->mask_url, $forbidden, true ) ) {
				...
				$this->errors[] = esc_html__( 'Redirect URL must different from Login URL', 'defender-security' );
			}
		}
	}
