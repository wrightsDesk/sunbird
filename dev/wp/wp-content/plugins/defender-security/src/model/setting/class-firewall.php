...
class Firewall extends Setting {
	...
	public function labels(): array {
		return array(
			'storage_days'                  => esc_html__( 'Days to keep logs', 'defender-security' ),
			'ip_blocklist_cleanup_interval' => esc_html__( 'Clear Temporary IP Block List', 'defender-security' ),
			'http_ip_header'                => esc_html__( 'Detect IP Addresses', 'defender-security' ),
			'trusted_proxies_ip'            => esc_html__( 'Edit Trusted Proxies', 'defender-security' ),
		);
	}
				...
				if ( ! $this->validate_ip( $ip ) ) {
					return array(
						...
						/* translators: %s: IP value. */
							esc_html__( '%s is not a valid IP address', 'defender-security' ),
							$ip
						),
					);
				}
