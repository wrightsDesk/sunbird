...
class Webauthn extends Controller {
	...
	public function create_challenge(): void {
		try {
			if ( ! $this->verify_nonce( 'wpdef_webauthn' ) ) {
				throw new Exception( esc_html__( 'Bad nonce.', 'defender-security' ) );
			}
	...
	public function remove_authenticator(): void {
		try {
			...
			if ( isset( $option_user_credentials[ $cred_id ] ) ) {
				...
				wp_send_json_success( esc_html__( 'Authenticator removed.', 'defender-security' ) );
			}

			wp_send_json_error( esc_html__( 'Key did not match any registered authenticator.', 'defender-security' ) );
		} catch ( Error $error ) {
			wp_send_json_error( $error->getMessage() );
		} catch ( Exception $exception ) {
			wp_send_json_error( $exception->getMessage() );
	...
	public function rename_authenticator(): void {
		try {
			...
			if ( isset( $option_user_credentials[ $cred_id ] ) ) {
				...
				wp_send_json_success( esc_html__( 'Authenticator identifier renamed.', 'defender-security' ) );
			}
	...
	public function get_credential_request_option(): void {
		try {
			...
			if ( ! is_array( $credential_sources ) || 0 === count( $credential_sources ) ) {
				throw new Exception( esc_html__( 'Please register a device first to authenticate it.', 'defender-security' ), 100 );
			}
	...
	public function get_translations(): array {
		$translations = array(
			...
			'authentication_start'               => esc_html__( 'Authenticating', 'defender-security' ),
			...
			'authenticator_verification_failed_user_handle_mismatch' => sprintf(
				...
				esc_html__( 'Due to some Biometric Authentication security improvements, the %s device will not work. Please delete this device and re-register it.', 'defender-security' ),
				'<strong>%s</strong>'
			),
